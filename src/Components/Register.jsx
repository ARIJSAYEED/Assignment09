    ...
    let handleSubmit = (e) => {
        ...
        createUser(email, password)
            .then(result => {
                ...
                updateUser({ diplayName: name, photoURL: photo })
                    .then(() => {
                        setUser({ ...user, diplayName: name, photoURL: photo })
                        navigate('/')
                    })
                    .catch(error => {
                        console.log(error);
            ...
            <div className="card bg-neutral-200 w-full max-w-lg shrink-0 shadow-lg">
                <div className="card-body px-10 space-y-5">
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div><Link to='/auth/login'>Already have an account? <span className='text-[#0099FF]'>Login Now!</span> </Link></div>
                    </form>
                </div>
            </div>
        </div>
